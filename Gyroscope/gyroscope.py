...
def PlottingGyroscope(time, theta, phi, x, y, z):
    ...
    fig.add_scatter3d(x=x[i:f],y=y[i:f],z=z[i:f], mode='lines',
...
def MatrixForcing(x0, w, t, forcing = "free"):
    """
    ...
    if forcing == "free":
        ...
        return smp.Matrix([x0 * smp.cos(w * t), 0, 0])
    
    elif forcing == "Y":
         return smp.Matrix([0, x0 * smp.cos(w * t), 0])

    if forcing == "XY":
        return smp.Matrix([x0 * smp.cos(w * t), x0 * smp.sin(w * t), 0])
...
def Solve_Gyro(time, CI, params, forcing = "free", plot = True, numbers = True):
    ...
    # Parameters settings
    t, h, g, m, x0, w, J1, J3, the, phi, psi, the_d, phi_d, psi_d, the_dd, phi_dd, psi_dd = ParametersDefinition()

    parametersSymbol = [t, h, g, m, x0, w, J1, J3]
    ...
    xc = h * R @ smp.Matrix([0,0,1]) + MatrixForcing(x0, w, t, forcing)
    xc.simplify()
    vc = smp.diff(xc, t)
    vc.simplify()
    ...
    if plot:
        PlottingGyroscope(t, the_t, phi_t, x_t, y_t, z_t)
...
def SwitchForcing(solution, newForcing, newTimeArray, params):
    ...
    newSolution = Solve_Gyro(newTimeArray, newCI, params, forcing = newForcing, plot = False, numbers = False)
...
def Get_Gyro_Position(t2, params_f, CI, t1=20):
    ...
    the_f, phi_f, psi_f,theD_f, phiD_f, psiD_f, path_f = Solve_Gyro_Forced_XY(T1, CI, params_f, plot=False)
