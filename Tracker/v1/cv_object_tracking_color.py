...
def real_coordindates(cood_obj, cood_cam = (0, 0), scale = (1, 1)):
    return [(cood_obj[i] - cood_cam[i]) * scale[i] for i in range(2)]
    ...
    try:
        ...
        while True:
            ...
            if isset('best_cnt'):
                ...
                x, y = real_coordindates((cx, cy))
                cv2.circle(frame,(cx,cy),5,255,-1)
                print("Central pos: (%d, %d)" % (cx,cy))
                end_time = time.time()
                with open(file_path, 'a') as file:
                    file.write(f"{end_time - start_time}; {x}; {IMAGE_HEIGHT - y} \n")
